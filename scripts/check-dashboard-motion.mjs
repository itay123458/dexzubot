...
try {
  ...
  await page.setContent('<section class="page active"><article class="card">Content</article></section><button class="icon-button loading"><svg><path d="M0 0h10"/></svg></button><div class="toast">Saved</div><div class="skeleton">Loading</div><i class="status-dot good"></i>');
  ...
  assert.notEqual(await animation('.page'), 'none', 'Page entry must animate with normal motion');
  ...
  assert.equal(await animation('.card'), 'none', 'Nested cards must not replay entry animations');
  assert.equal(await animation('.status-dot'), 'none', 'Idle status indicators should stay still');
  await page.emulateMedia({ reducedMotion: 'reduce' });
  for (const selector of ['.page', '.loading svg', '.toast', '.skeleton']) assert.equal(await animation(selector), 'none');

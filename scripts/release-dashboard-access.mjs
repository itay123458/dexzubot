...
import { dashboardAccessRelease as release } from '../src/config/releases.js';
const beta='1486680755869323388',channelId='1549857857414111272';
if(process.env.BETA_GUILD_ID!==beta||process.env.GUILD_ID!=='1533088766821007390')throw Error('Expected DexzuBot configuration.');
const client=new Client({intents:[GatewayIntentBits.Guilds]});client.db=db;
try {
 await initializeDatabase();if(!db.isAvailable())throw Error('Database unavailable.');
 ...
 const payload={content:'',embeds:[createEmbed({guildId:beta,title:release.title,author:'DEXZUBOT / UPDATES',description:[release.summary,...release.changes.map(text=>`• ${text}`)].join('\n\n'),fields:[{name:'Try it',value:release.tryIt.join('\n')}],footer:'DexzuBot · Dashboard access'})],allowedMentions:{parse:[]}};

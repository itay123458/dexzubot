...
try {
  ...
  assert.deepEqual(edits[0].embeds[0].toJSON().fields.map(f => f.name), ['/ping', '/slash']);
  let rejected = false;
  ...
  assert.equal(edits.at(-1).embeds[0].toJSON().fields?.length || 0, 0, 'Existing help menu must refresh after category is disabled');
  let denied = false;

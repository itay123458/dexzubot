...
try {
  ...
  assert.ok(ended.description.includes('<@123456789012345678>'));
  const active = createGiveawayEmbed(giveaway, 'active').toJSON();
  assert.equal(active.footer, undefined);
  assert.equal(active.fields, undefined);
  assert.equal(active.description.split('\n').length, 3);
  assert.ok(active.description.includes(':R>'), 'Active giveaway retains its deadline');
  ...
  assert.equal(withFiles.files, undefined);
  assert.equal(withFiles.embeds[0].toJSON().image, undefined);
  ...
  assert.deepEqual(retained.attachments, [], 'Remove the old banner on edits');
  assert.equal(retained.embeds[0].toJSON().image, undefined);
  const otherFile = giveawayService.withGiveawayArtwork(createGiveawayEmbed(giveaway, 'active'), channel(true), {
    attachments: new Map([['old', { id: 'old', name: 'dexzu-giveaway-v2.png' }], ['keep', { id: 'keep', name: 'rules.txt' }]]),
  });
  assert.deepEqual(otherFile.attachments, [{ id: 'keep' }], 'Preserve unrelated attachments');

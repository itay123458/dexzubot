...
try {
  ...
  assert.match(reply.embeds[0].toJSON().description, /1\/24 discovered/);
  await button.execute({ ...interaction, user: { id: '123456789012345678' }, customId: `eat_collection:${userId}` }, client, [userId]);
  assert.match(reply.embeds[0].toJSON().description, /0\/24 discovered/, 'button ignores forged target and reads clicking member');
